...
const languages_manager_1 = require("../server/utils/lang/languages-manager");
const request_handler_1 = require("../server/utils/jdoodle-request-handler/request-handler");
class AppRoutes {
    constructor() {
    ...
    initRoutes() {
        this.routes
            ...
            const langs = languages_manager_1.LanguagesManager.getLanguagesMap();
            console.log('rout: ', langs);
            return res.status(200).send([...langs]);
        })
            .post('/run', (req, res) => {
            console.log({ msg: 'POST: \'/run\'' });
            const body = _.pick(req.body, ['lang', 'version', 'program']);
            if (!this.isValidRunBody(body)) {
